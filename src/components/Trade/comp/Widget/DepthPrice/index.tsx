...
import { getPrecisionByNumber } from '@/utils/number'
...
function DeepPrice() {
    ...
    <div className="border-t border-b border-gray-60 py-2 px-3">
      <div className="flex items-center justify-between">
        <div>
          {/* 当前行情卖价 */}
          {quote.hasQuote ? (
            <span className={cn('text-lg font-pf-bold pr-[10px]', quote.bidDiff > 0 ? 'text-green' : 'text-red')}>
              {formatNum(quote.bid)}
            </span>
          ) : (
        ...
        return (
          <div key={idx} className="relative overflow-hidden" style={{ animation: '0.2s ease-out 0s 1 normal none running none' }}>
            <Row className="flex items-center h-6 px-3 relative z-[2]">
              ...
              <Col span={8} className="font-medium !text-xs text-primary text-left">
                {amountDigits >= 8 ? formatNum(item.amount, { precision: 8 }) : formatNum(item.amount)}
              </Col>
              <Col span={8} className="font-medium !text-xs text-primary text-right">
  ...
  return (
    <div className={cn('w-[260px] h-[700px] overflow-hidden bg-white relative flex-shrink-0', className)}>
