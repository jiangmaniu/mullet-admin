...
import React, { forwardRef, useEffect, useImperativeHandle, useRef, useState, useTransition } from 'react'
  ...
  const renderSearch = () => (
    <div className="px-3 pt-3">
      <Input
        ...
        allowClear
        style={{ background: 'var(--input-bg)', height: 36 }}
      ...
      pcComponent={
        <>
          ...
          {(openTradeSidebar || !showFixSidebar) && (
            <div className={cn('h-[700px] w-[364px] flex-shrink-0 bg-white relative', { [borderClassName]: showFixSidebar })} style={style}>
              {renderTabs()}
              {renderSearch()}
              {renderCategoryTabs()}
          ...
          {/* 收起侧边栏视图 */}
          {!openTradeSidebar && showFixSidebar && (
            <div className={cn('h-[700px] w-[60px] bg-white flex flex-col items-center relative', borderClassName)}>
