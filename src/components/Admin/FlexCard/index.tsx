...
export default function StatisticCard({
  ...
  const { lng } = useLang()
  const proCardStyle = cardProps?.style || ({} as React.CSSProperties)
  if (showCardBg) {
  ...
  const renderItem = (item: Item) => {
    return (
      <>
        ...
        <span className="text-gray-secondary" style={{ fontSize: labelFontSize, lineHeight, ...labeStyle }}>
          {item.label}
        </span>
      </>
    )
  ...
  return (
    <div
      style={{
        ...
        overflowY: 'hidden',
        display: 'flex',
        flexDirection: cardProps?.direction || 'row',
        flexWrap: cardProps?.wrap ? 'wrap' : 'nowrap',
        width: '100%',
        ...style,
        ...proCardStyle
      }}
