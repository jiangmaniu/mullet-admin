...
import { Form, FormItemProps, Input, InputNumber, InputNumberProps, InputProps, Select, SelectProps } from 'antd'
    ...
    addonAfter: (
      <Form.Item noStyle>
        <Select
          ...
          options={[
            {
              label: intl.formatMessage({ id: 'mt.yushe' }),
              value: '',
              disabled: true
            },
            ...options
          ...
          labelRender={(props) => <span className="text-sm">{intl.formatMessage({ id: 'mt.yushe' })}</span>}
          size="large"
          {...selectProps}
        />
      </Form.Item>
  ...
  return (
    <>
      <Form.Item
        ...
        {...formItemProps}
      >
        {type === 'number' ? <InputNumber controls={false} {...commonProps} {...inputProps} /> : <Input {...commonProps} {...inputProps} />}
      </Form.Item>
    </>
  )
}
