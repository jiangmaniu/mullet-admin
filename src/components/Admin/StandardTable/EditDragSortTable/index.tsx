...
import { ButtonProps, ConfigProvider, Popconfirm, TablePaginationConfig } from 'antd'
import { NamePath } from 'antd/es/form/interface'
import { FormInstance } from 'antd/lib'
import { cloneDeep } from 'lodash'
import React, { forwardRef, ReactNode, useCallback, useContext, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react'
...
import { useDragSort } from '../BaseDragSortTable/useDragSort'
...
export type EditDragSortTableRef = {
  /**取消编辑 */
  cancelEditable?: () => void
...
const EditableTable = <T extends Record<string, any>, U extends ParamsType>(
  ...
  ref: React.Ref<EditDragSortTableRef>
) => {
  ...
  const editableKeysRef = useRef<any>([])
  ...
  const DragHandle = useMemo(() => {
    return (dragHandleProps: any) => {
      ...
      const defaultDom = (
        <Iconfont name="tuodong" {...rest} className={`${getPrefixCls('pro-table-drag-icon !size-8')} ${className || ''}`.trim()} />
      )
  ...
  const onDragEndBefore = useCallback(() => {
    if (editableKeysRef?.current?.length > 0) {
      message.info(intl.formatMessage({ id: 'common.editTableSortTips' }))
      return false
    }
    return true
  }, [intl])
  ...
  const { components, DndContext } = useDragSort<T>({
    dataSource: useFormMode ? [] : dataSource?.slice(),
    dragSortKey,
    onDragSortEnd,
    components: {},
    rowKey: rowKey || 'id',
      ...
      <EditableProTable<T, U>
        ...
        editable={{
            ...
            return [
              ...
              >
                ...
                <a
                  ...
                  className="flex items-center !text-primary"
                >
                  <Iconfont name="quxiao" className="!size-5 !text-primary" />
          ...
          onCancel: async (key, row, originRow) => {
            ...
            console.log('onCancel', key, rowData, originRow)
