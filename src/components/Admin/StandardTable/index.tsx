...
import { forwardRef, ForwardRefRenderFunction, ReactNode, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react'
...
export type StandardTableRef = Instance
...
export interface IStandardTableProps<T, U> extends Omit<ProTableProps<T, U>, 'options' | 'search'> {
  /**@name 表格 */
  ...
  /**修复在弹窗中搜索表单样式问题*/
  fixSearchFormStyleInModal?: boolean
  /**表单、表格等实例 */
  ...
  /**操作栏表头前渲染 */
  getOpColumnBeforeItems?: (record: T, actionRef: ActionType, formRef?: FormInstance) => ReactNode
  /**操作栏表头宽度 */
  ...
  /** @name 重置表单时触发 */
  onReset?: () => void
  /**在表格上方渲染元素，和table通用一个proCard布局，区别于tableExtraRender在表格上方渲染但是不能跟table公用一个proCard */
  renderTableHeader?: () => React.ReactNode
  /**搜索前参数统一处理入口 */
  beforeSearchSubmit?: (params: any) => void
}

const StandardTableComp: ForwardRefRenderFunction<StandardTableRef, IStandardTableProps<any, any>> = <
...
>(
  {
    ...
    fixSearchFormStyleInModal = false,
    getInstance,
    bodyStyle,
    headStyle,
    className,
    getOpColumnItems,
    getOpColumnBeforeItems,
  ...
  ref: React.Ref<StandardTableRef>
) => {
        ...
        render: (text, record, index, _action) => {
          ...
          return [
            <div key={record.id || index} className={optionColumnClassName}>
              {getOpColumnBeforeItems?.(record, actionRef.current as ActionType, formRef.current as FormInstance)}
  ...
  return (
    <ProTable<T, U>
      ...
      beforeSearchSubmit={(params) => {
        ...
        // 使用日期，不需要时分秒格式
        if (dateRangeNoTime && dateRangeNoTime[0] && dateRangeNoTime[1]) {
          // @ts-ignore
      ...
      className={cn(
        ...
        { 'table-in-modal': fixSearchFormStyleInModal },
        className,
...
// 修改导出方式
const StandardTable = forwardRef(StandardTableComp) as <T extends Record<string, any>, U extends ParamsType = ParamsType>(
  props: IStandardTableProps<T, U> & { ref?: React.Ref<StandardTableRef> }
) => React.ReactElement
