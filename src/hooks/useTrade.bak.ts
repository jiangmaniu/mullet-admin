...
import { getPrecisionByNumber, toNegativeOrEmpty } from '@/utils/number'
import { calcExchangeRate, calcExpectedMargin, getCurrentQuote, getMaxOpenVolume } from '@/utils/wsUtil'
...
export default function useTrade(props?: IProps) {
  ...
  const maxOpenVolume = trade.leverageMultipleMaxOpenVolume || getMaxOpenVolume({ buySell }) // 最大可开手数
  ...
  // 实时计算下单时预估保证金
  // 实时计算下单时预估保证金
  const marginOrderType = useMemo(() => {
    if (orderType === 'MARKET_ORDER') {
      return 'MARKET_ORDER'
    } else if (orderType === 'LIMIT_ORDER') {
      return isBuy ? ORDER_TYPE.LIMIT_BUY_ORDER : ORDER_TYPE.LIMIT_SELL_ORDER
    } else if (orderType === 'STOP_LIMIT_ORDER') {
      return isBuy ? ORDER_TYPE.STOP_LOSS_MARKET_BUY_ORDER : ORDER_TYPE.STOP_LOSS_MARKET_SELL_ORDER
    }
    return ''
  }, [orderType, isBuy])

  const expectedMargin = useMemo(() => {
    return calcExpectedMargin({
      buySell,
      orderVolume: Number(orderVolume),
      orderType: marginOrderType,
      price: orderType === 'MARKET_ORDER' ? undefined : orderPrice
    })
  }, [buySell, orderVolume, marginOrderType, orderType, orderPrice, quoteInfo])
  ...
  const priceTip = useMemo(() => {
    ...
    return 0
  }, [orderType, isBuy, stopll, d, currentPrice, isPosition])
