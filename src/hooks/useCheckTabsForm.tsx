  ...
  const checkForm = async () => {
    ...
    if (errors.length) {
      setTimeout(() => {
        api?.open?.({
          message: (
            <div className="text-primary font-semibold text-base">{getIntl().formatMessage({ id: 'mt.qingwanshanbitianxiang' })}</div>
          ),
          // duration: 0,
          description: (
            <div
              ...
              {errors.map((item: any, idx) => (
                ...
                >
                  ...
                  {item.formErrors.map((v: any, index: number) => (
                    <div key={index} className="text-secondary text-sm" style={{ lineHeight: '24px' }}>
                      {v.message}
                    </div>
                  ))}
                </div>
    ...
    if (errors.length === 0 && Object.keys(formData).length) {
      ...
      if (startTime && endTime) {
        // @ts-ignore
        formData.startTime = formatTime(startTime)
        // @ts-ignore
