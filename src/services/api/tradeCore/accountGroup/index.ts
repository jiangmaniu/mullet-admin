  ...
  return request<API.Response<AccountGroup.AccountGroupPageListItem>>('/api/trade-core/coreApi/accountGroup/detail', {
    ...
    if (res.success && res.data) {
      ...
      if (synopsis && typeof synopsis === 'string') {
        // 解析账户介绍内容回显
        res.data.synopsis = JSON.parse(synopsis)
        if (!Array.isArray(res.data.synopsis)) {
          // 默认创建一条
          res.data.synopsis = [
            {
              language: 'zh-TW', // 语言编码
...
// 交易账户组-克隆
export async function cloneAccountGroup(body: AccountGroup.CloneAccountGroupParams) {
  return request<API.Response>('/api/trade-core/coreApi/accountGroup/clone', {
    method: 'POST',
    data: body
  })
}
  ...
  return request<API.Response<API.PageResult<AccountGroup.AccountGroupSymbolPageListItem>>>(
    ...
    if (res.data?.records?.length) {
      res.data.records = res.data.records.map((item: any) => {
        if (item.symbolConf) {
          parseJsonFields(item, ['symbolConf'])
          item.symbolConf = formatSymbolConf(item.symbolConf)
        }
        return item
      })
    }
