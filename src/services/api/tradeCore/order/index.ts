...
import { getBgaOrderPriceProfit } from '../../crmManage/client'
...
export const formatOrderResult = (res: any) => {
...
// 更新持仓订单数据源,构造标记价格和盈亏
export const updateBgaOrderDataSource = async (dataRaw: Order.BgaOrderPageListItem[]) => {
  const list = dataRaw.filter((item: any) => item.status === 'BAG') || []
  const asgoArrayStr = list.map((item: any) => `${item.tradeAccountId}-${item.symbol}-${item.accountGroupId}-${item.id}`).join(',')

  if (asgoArrayStr?.length > 0) {
    const res = await getBgaOrderPriceProfit({
      asgoArrayStr
    })

    if (res.success) {
      dataRaw.forEach((item: any) => {
        const dataKey = `${item.tradeAccountId}-${item.symbol}-${item.accountGroupId}-${item.id}`
        const currentQuote = res.data?.[dataKey]?.price
        if (currentQuote) {
          item.currentQuote = currentQuote
          item.profit = res.data?.[dataKey]?.profit // 获取持仓中的盈亏
        }
      })
    }
  }
...
// 【全仓】追加预付款账户【敏感账户】
export async function getOrderCrossHighList(params?: any) {
  return request<API.Response<Order.CrossHightListItem[]>>('/api/trade-core/coreApi/orders/crossHigh', {
    method: 'GET',
    params
  })
}
...
// 【逐仓】追加预付款订单【敏感订单】
export async function getOrderIsolatedHighList(params?: any) {
  return request<API.Response<Order.IsolatedHighListItem[]>>('/api/trade-core/coreApi/orders/isolatedHigh', {
    method: 'GET',
    params
  })
}
