...
class WSStore {
  ...
  subscribeExchangeRateQuote = (symbolConf?: Symbol.SymbolConf, symbolName?: string) => {
    ...
    const quote = getCurrentQuote(activeSymbolName)
    // 如果不传，使用当前激活的品种配置
  ...
  symbolToString = (symbol: SymbolWSItem) => {
    // 如果 accountGroupId 存在，则使用 accountGroupId，否则使用 dataSourceCode
    return symbol.accountGroupId ? `${symbol.symbol}-${symbol.accountGroupId}` : `${symbol.symbol}-${symbol.dataSourceCode}`
  }
