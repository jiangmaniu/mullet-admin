...
export const formatNum = (value: any, opts?: IOpt) => {
  ...
  const _precision = opts?.precision
  let precision = _precision && Number.isInteger(_precision) ? _precision : String(value).split('.')?.[1]?.length || DEFAULT_PRECISION
  // 防止精度过大
  if (precision >= 10) {
    precision = 10
  }
  const truncateValue = isTruncateDecimal ? truncateDecimal(val, precision) : val // 截取小数点，不四舍五入
  return currency(truncateValue, { ...opts, symbol: '', precision }).format() + unit
}
...
// 复制功能
export const copyContent = (cotVal: string, title = 'Copy Success') => {
