...
import { formatMin2Time, formatNum, getUid, groupBy, isImageFile, parseJsonFields } from '.'
...
export function transformTradeInventorySubmit(conf: Symbol.HoldingCostConf) {
  ...
  conf.isEnable = conf?.isEnable ? true : false
  conf.isHoliday = conf.isHoliday ? true : false
...
export const formatSymbolConf = (data: any) => {
  ...
  if (symbolConf) {
    ...
    symbolConf.depthOfMarket = Number(symbolConf.depthOfMarket)
  }
  return symbolConf
}
 ...
 * @returns
 */
export const formatValueByField = (info: any, fields: any) => {
  fields.forEach((key: string) => {
    info[key] = formatNum(info[key])
  })
  return info
}
