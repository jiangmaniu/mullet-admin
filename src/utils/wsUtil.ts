...
import { IPositionItem } from '@/components/Trade/comp/TradeRecord/comp/PositionList'
...
import { getPrecisionByNumber } from './number'
...
export const calcExchangeRate = ({ value, unit, buySell }: IExchangeRateParams) => {
  ...
  if (trade.currentAccountInfo?.currencyUnit !== unit && unit) {
    // USD开头是除法，USD结尾是乘法
    // 除法
    ...
    console.log('divNameKey', divNameKey)
    console.log('mulNameKey', mulNameKey)
 ...
 * @returns
 */
export function covertProfit(positionItem: Order.BgaOrderPageListItem, opts?: IOptions) {
  const symbol = positionItem?.symbol
  if (!symbol) return

  const quoteInfo = getCurrentQuote(opts?.symbol ? opts : symbol)
  ...
  // @ts-ignore
  const digits = symbolConf?.symbolDecimal || 2 // 小数位
...
export function getCurrentDepth(currentSymbolName?: string) {
  ...
  const { dataSourceCode, accountGroupId } = trade.getActiveSymbolInfo(symbol)
 ...
 * @param currentSymbolName 当前传入的symbolName或配置
 * @returns
 */
type IOptions = {
  /**品种名称 */
  ...
  /**使用行情推送过来的精度 */
  useQuoteDigits?: boolean
  /**是否包含手续费 */
  includeFee?: boolean
}
export function getCurrentQuote(currentSymbolName?: string | IOptions) {
  const { ws, trade } = stores
  const { quotes } = ws
  const symbolTicker = trade.symbolTicker
  let symbol = ''
  let opts = {} as IOptions

  // 处理传入不同参数类型的情况
  if (currentSymbolName && typeof currentSymbolName === 'object') {
    opts = currentSymbolName
    symbol = opts?.symbol
  } else {
    symbol = currentSymbolName || trade.activeSymbolName // 展示的名称(后台自定义的品种名称)
  }

  // 当前品种的详细信息
  const currentSymbol = trade.symbolMapAll?.[symbol] || {}
  const dataSourceSymbol = currentSymbol?.dataSourceSymbol as string
  const dataSourceCode = opts?.dataSourceCode
  ...
  const currentQuote = opts?.currentQuote || quotes.get(dataSourceKey) // 行情信息
  ...
  // symbolTicker 根据数据源获取的高开低收，用于数据源列表，currentSymbol?.symbolNewTicker品种列表中的高开低收，用于交易页面品种展示
  const symbolNewTicker =
    opts?.from === 'DataSourceList'
      ? (symbolTicker[symbol] as DataSource.SymbolTicker)
      : trade.tradeSymbolTickerMap[symbol] || currentSymbol?.symbolNewTicker // 品种高开低收 高开低收价格信息，只加载一次，不会实时跳动，需要使用ws的覆盖
  const symbolNewPrice = currentSymbol.symbolNewPrice // 第一口报价信息，只加载一次，不会实时跳动，需要使用ws的覆盖

  // 获取行情价格精度
  const quoteDigits = currentQuote?.priceData?.buy ? getPrecisionByNumber(currentQuote?.priceData?.buy) : 2
  const digits = opts?.useQuoteDigits ? quoteDigits : opts?.digits || Number(currentSymbol?.symbolDecimal || 2) // 小数位，默认2
