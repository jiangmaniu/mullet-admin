    ...
    <PageContainer icon="/img/emoji/6.png" pageBgColorMode="gray">
      <StandardTable
        ...
        onEditItem={(record) => {
          ...
          push(`/account-group/demo/edit/${record.id}`)
        }}
        tableExtraRender={() => (
          <div className="flex gap-3">
            <AddButton href="/account-group/demo/add">
        ...
        getOpColumnBeforeItems={(record, actionRef, formRef) => {
          return (
            <a
              onClick={async () => {
                push(`/account-group/demo/clone?id=${record.id}`)
              }}
        ...
        // ghost
        action={{
          query: (params) => getAccountGroupPageList({ ...params, isSimulate: true }),
