  ...
  return (
    <EditableTable
      ...
      checkRowSaveBefore={(record) => {
        if (!record.symbolGroupId) {
          message.info(getIntl().formatMessage({ id: 'mt.pinzhongleibiebunengweikong' }))
          return true
        }
        if (!record.rewardAmount) {
          message.info(getIntl().formatMessage({ id: 'mt.zengjinbunengweikong' }))
          return true
        }
        return false
      }}
