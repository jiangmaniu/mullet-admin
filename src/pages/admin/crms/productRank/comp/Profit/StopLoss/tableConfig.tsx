...
export const getColumns = (): ProColumns<CrmTrading.LossItem>[] => {
  return [
    {
    ...
    {
      title: <FormattedMessage id="mt.kuisunjine" />,
      dataIndex: 'profit',
      hideInSearch: true, // 在 table的查询表单 中隐藏
      ellipsis: false,
      align: 'right',
      ...
      renderText(text, record, index, action) {
        ...
        return <span className="text-red">{text && text !== '0' ? `${numberFormatUnit(val, { precision: DEFAULT_PRECISION })}` : '0'}</span>
      },
      width: 100
    }
  ]
