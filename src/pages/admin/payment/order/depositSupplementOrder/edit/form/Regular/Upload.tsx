...
const App: React.FC<IProps> = ({ disabled, label, required, name, initialValues }) => {
  ...
  useEffect(() => {
    if (initialValues?.voucherPicture) {
      setFileList(
        initialValues.voucherPicture.split(',').map((item) => ({
          uid: getUid(),
          name: item,
          status: 'done',
    ...
    <>
        ...
        rules={[
          {
            ...
            validator(rule, value, callback) {
              if (!fileList.filter((item) => item.status === 'done').length) {
                return Promise.reject(intl.formatMessage({ id: 'mt.shangchuanzhuanzhangpinzhengtupian' }))
              }
      ...
      >
        <Upload
          ...
          disabled={disabled}
        >
          {disabled || fileList.length >= 4 ? null : uploadButton}
        </Upload>
      </Form.Item>
      {previewImage && (
        <Image
