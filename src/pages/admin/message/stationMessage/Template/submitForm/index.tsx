...
import { getMessageTemplateDetail, submitMessageTemplate } from '@/services/api/message/messageTemplate'
...
export default function Add() {
  ...
  const { loading, data, run } = useRequest(getMessageTemplateDetail, { manual: true })
  ...
  return (
    <PageContainer
      ...
      pageTitle={<>{isAdd ? <FormattedMessage id="mt.xinjianzhanneixinmu" /> : <FormattedMessage id="mt.bianjizhanneixinmu" />}</>}
      renderRight={() => (
        <SaveButton
          onClick={async () => {
            // 校验表单
            ...
            if (formData) {
              ...
              setSaveLoading(true)
              const res = await submitMessageTemplate(values)
              if (res.success) {
