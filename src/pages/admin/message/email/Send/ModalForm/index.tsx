...
import { submitEmailSend } from '@/services/api/email/emailSend'
...
function ModalForm({ trigger, info = {}, reload, onClose }: IProps, ref: any) {
  ...
  return (
    <>
      <Modal
        title={isAdd ? <FormattedMessage id="mt.xinzengfasongyoujian" /> : <FormattedMessage id="mt.chakanfasongyoujian" />}
        onFinish={async (values: any) => {
          ...
          const res = await submitEmailSend(params)
      ...
      >
        <div className="grid gap-y-5 my-5 gap-x-7 grid-cols-2">
          ...
          {/* <ProFormSelect
            label={intl.formatMessage({ id: 'mt.fasongtongdao' })}
            required={isAdd}
            name="sendChannel"
            options={[
              { label: "阿里云", value: '1' },
