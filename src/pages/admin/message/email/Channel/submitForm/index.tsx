...
import { getEmailChannelDetail, submitEmailChannel } from '@/services/api/email/emailChannel'
...
export default function Add() {
  ...
  const { loading, data, run } = useRequest(getEmailChannelDetail, { manual: true })
  ...
  return (
    <PageContainer
      ...
      pageTitle={<>{isAdd ? <FormattedMessage id="mt.xinjiantongdao" /> : <FormattedMessage id="mt.bianjitongdao" />}</>}
      renderRight={() => (
        <SaveButton
          onClick={async () => {
            // 校验表单
            ...
            // 表单校验成功
            if (formData) {
              const { configInfo, ...params } = formData as any
              const values = {
                ...params,
                // 数组对象转对象
              ...
              setSaveLoading(true)
              const res = await submitEmailChannel(values)
              if (res.success) {
